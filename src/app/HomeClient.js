...
const getProductAltText = (product, color = 'Default') => {
  const name = product.name || 'Product';
  const lowercaseName = name.toLowerCase();
  
  let type = 'merchandise';
  if (lowercaseName.includes('tee') || lowercaseName.includes('t-shirt') || lowercaseName.includes('shirt')) {
    type = 'tee';
  } else if (lowercaseName.includes('mug') || lowercaseName.includes('jug') || lowercaseName.includes('flask')) {
    type = 'mug';
  } else if (lowercaseName.includes('socks')) {
    type = 'socks';
  } else if (lowercaseName.includes('cap')) {
    type = 'cap';
  } else if (lowercaseName.includes('poster')) {
    type = 'poster';
  } else if (lowercaseName.includes('bag') || lowercaseName.includes('tote')) {
    type = 'tote bag';
  } else if (lowercaseName.includes('notepad')) {
    type = 'notepad';
  } else if (lowercaseName.includes('cushion')) {
    type = 'cushion cover';
  } else if (lowercaseName.includes('sticker')) {
    type = 'sticker pack';
  }
  
  let colorStr = color !== 'Default' ? color : '';
  if (!colorStr) {
    if (lowercaseName.includes('black')) colorStr = 'Black';
    else if (lowercaseName.includes('white')) colorStr = 'White';
    else colorStr = 'unbothered color';
  }
  
  const gsmStr = (type === 'tee' || lowercaseName.includes('hoodie') || lowercaseName.includes('armour')) ? ' — 240 GSM cotton' : '';
  
  return `${name} ${type} — ${colorStr}${gsmStr} India`;
};

export default function HomeClient({ initialProducts = [], initialFeaturedCategories = [] }) {
  const { user } = useAuth();
  ...
  const [featuredCategories, setFeaturedCategories] = useState(initialFeaturedCategories);
  ...
  // Load Products client-side if server fetch returned empty (as a fallback)
  useEffect(() => {
    if (initialProducts.length > 0) {
      setProducts(initialProducts);
      setFeaturedCategories(initialFeaturedCategories);
    ...
    async function loadProducts() {
      try {
        ...
        console.error('Failed to load products client-side:', err);
      } finally {
        setLoadingProducts(false);
      }
    }
    loadProducts();
  }, [initialProducts, initialFeaturedCategories]);
    ...
    <div className="vintage-grain min-h-screen py-6 sm:py-10 px-4 md:px-8 max-w-7xl mx-auto flex flex-col gap-8 sm:gap-16 w-full overflow-hidden">
              ...
              {loadingProducts ? (
                ...
                products
                  ...
                  .map(product => (
                    ...
                    >
                      {/* Title */}
                      <Link href={`/products/${product.id}`} className="border-b-2 border-black p-2 bg-white hover:text-[#C2410C] block">
                      ...
                      {/* Image */}
                      <Link href={`/products/${product.id}`} className="relative aspect-square w-full overflow-hidden bg-white block">
                        <Image
      ...
      <section className="border-4 border-black bg-[#EAE5D9] p-4 sm:p-8 md:p-12 flex flex-col lg:flex-row gap-6 sm:gap-8 items-center w-full">
        <div className="lg:w-2/3 w-full flex flex-col gap-4 sm:gap-6">
          ...
          {/* Trust Badges moved above the fold in hero section */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-[10px] font-bold uppercase text-gray-700 border-t border-black/10 pt-4">
            <span>🔒 Secured UPI</span>
            <span>📦 Ships in 3–5 days</span>
            <span>↩️ Easy returns</span>
            <span>📏 Size chart available</span>
          </div>
        </div>
        ...
        <div className="border-4 border-black p-4 bg-black flex flex-col gap-3 text-[#EAE5D9] shadow-2xl lg:w-1/3 w-full max-w-sm lg:max-w-none mx-auto">
          <div className="relative aspect-[3/4] w-full border-2 border-[#EAE5D9] overflow-hidden">
            <Image
              src="/homepage.webp"
              alt="Cockroach India Store funny graphic tee — anti-work satirical design"
              fill
              unoptimized
      ...
      <section className="border-4 border-black bg-black text-[#EAE5D9] p-2 sm:p-4 w-full">
        <div className="relative aspect-video w-full border-2 border-[#EAE5D9] overflow-hidden">
          <Image
            src="/banner.webp"
            alt="Cockroach India Store — heavy cotton tees and mugs from ₹499, shipped across India"
            fill
            className="object-cover"
          />
        </div>
              ...
              return (
                <div key={product.id} className="border-4 border-black bg-[#EAE5D9] flex flex-col justify-between shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-all">
                  ...
                  <div className="p-4 flex-grow flex flex-col justify-between gap-4">
                    <div>
                      <Link href={`/products/${product.id}`} className="font-display text-sm uppercase font-black hover:text-[#C2410C] leading-tight block">
                        {product.name}
                      </Link>
      ...
      {!user && (
        <section className="border-4 border-[#C2410C] bg-[#EAE5D9] p-4 sm:p-8 text-center flex flex-col gap-4 items-center">
          ...
          <p className="text-xs font-semibold max-w-lg leading-relaxed">
            Create a free account for early access to new drops. Manage your delivery details and access members-only limited stock drops.
          </p>
          <Link
            href="/register"
